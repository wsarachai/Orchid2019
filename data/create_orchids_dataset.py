...
flags.DEFINE_string(
    "images_dir", "/Volumes/Data/_dataset/_orchids_dataset/orchids52_data", "Original orchid flower images directory"
)

flags.DEFINE_string(
    "output_directory", "/Volumes/Data/_dataset/_orchids_dataset/orchids52_data/tf-records/v1", "Output data directory"
)

flags.DEFINE_string("data_version", ORCHIDS52_DATA_V1, "The version of dataset")
...
def _find_image_files_v1(images_dir, image_size):
    ...
    _process_path = wrapped_partial(process_path, class_names=class_names, image_size=image_size)
...
def _write_tf_file(tf_file, ds):
    ...
    with tf.io.TFRecordWriter(tf_file) as writer:
        ...
        for counter, (image, label) in enumerate(ds):
            ...
            sys.stdout.write(
                "\r>> {} [{}]: Processed {} of {} images in thread batch.".format(
                    datetime.now(), parts[-1], counter, num_files
                )
            )
            sys.stdout.flush()


def _create_dataset(images_dir, output_directory, image_size):
    train_ds = None
    test_ds = None
    validate_ds = None

    if FLAGS.data_version == ORCHIDS52_DATA_V1:
        train_ds, test_ds, validate_ds = _find_image_files_v1(images_dir=images_dir, image_size=image_size)
    elif FLAGS.data_version == ORCHIDS52_DATA_V2:
        train_ds, test_ds, validate_ds = _find_image_files_v2(images_dir=images_dir, image_size=image_size)

    if not tf.io.gfile.exists(output_directory):
        tf.io.gfile.mkdir(output_directory)

    if train_ds:
        _write_tf_file(tf_file=os.path.join(output_directory, "orchids52-train.tfrecord"), ds=train_ds)
    if test_ds:
        _write_tf_file(tf_file=os.path.join(output_directory, "orchids52-test.tfrecord"), ds=test_ds)
    if validate_ds:
