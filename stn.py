...
class SpatialTransformerNetwork(tf.keras.layers.Layer):
    ...
    def call(self, inputs, scale, training=None, **kwargs):
        if training is None:
            training = K.learning_phase()

        scale_mat = tf.constant(scale, shape=(self.batch_size, 1), dtype=tf.float32)

        theta = kwargs.get("theta", None)
        bound_err = []

        theta, bound_err = get_parameter(self.batch_size, theta, self.x_zero, self.y_zero, scale_mat)
        ...
        _theta = tf.squeeze(theta, axis=1)
        ...
        if training and self.trainable:
            image_update_s = tf.function(k_summary.image_update).get_concrete_function(
                name="stn/image/{}".format(str(scale)), unit=out_fmap, max_outputs=3
            )
