...
def ResNet(
    ...
    **kwargs
    ...
    x = layers.ZeroPadding2D(padding=((3, 3), (3, 3)), name="%s_conv1_pad" % name_prefix)(img_input)
    x = layers.Conv2D(64, 7, strides=2, use_bias=use_bias, name="%s_conv1_conv" % name_prefix)(x)

    if not preact:
        x = layers.BatchNormalization(axis=bn_axis, epsilon=1.001e-5, name="%s_conv1_bn" % name_prefix)(x)
        x = layers.Activation("relu", name="%s_conv1_relu" % name_prefix)(x)

    x = layers.ZeroPadding2D(padding=((1, 1), (1, 1)), name="%s_pool1_pad" % name_prefix)(x)
    x = layers.MaxPooling2D(3, strides=2, name="%s_pool1_pool" % name_prefix)(x)

    x = stack_fn(x)

    if preact:
        x = layers.BatchNormalization(axis=bn_axis, epsilon=1.001e-5, name="%s_post_bn" % name_prefix)(x)
        x = layers.Activation("relu", name="%s_post_relu" % name_prefix)(x)

    if include_top:
        x = layers.GlobalAveragePooling2D(name="%s_avg_pool" % name_prefix)(x)
        imagenet_utils.validate_activation(classifier_activation, weights)
        x = layers.Dense(classes, activation=classifier_activation, name="%s_predictions" % name_prefix)(x)
    else:
        if pooling == "avg":
            x = layers.GlobalAveragePooling2D(name="%s_avg_pool" % name_prefix)(x)
        elif pooling == "max":
            x = layers.GlobalMaxPooling2D(name="%s_max_pool" % name_prefix)(x)
...
@keras_export(
    "keras.applications.resnet50.ResNet50", "keras.applications.resnet.ResNet50", "keras.applications.ResNet50"
)
def ResNet50(
    ...
    return ResNet(
        stack_fn, False, True, "resnet50", include_top, weights, input_tensor, input_shape, pooling, classes, **kwargs
    )
...
def ResNet101(
    ...
    return ResNet(
        stack_fn, False, True, "resnet101", include_top, weights, input_tensor, input_shape, pooling, classes, **kwargs
    )
...
def ResNet152(
    ...
    return ResNet(
        stack_fn, False, True, "resnet152", include_top, weights, input_tensor, input_shape, pooling, classes, **kwargs
    )
...
preprocess_input.__doc__ = imagenet_utils.PREPROCESS_INPUT_DOC.format(
    mode="", ret=imagenet_utils.PREPROCESS_INPUT_RET_DOC_CAFFE, error=imagenet_utils.PREPROCESS_INPUT_ERROR_DOC
)
