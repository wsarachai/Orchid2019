...
from nets.resnet_v2_50 import Orchids52Resnet50
...
from nets.layers import BranchBlockResnet50
...
class Orchids52Resnet50STN(Orchids52Resnet50):
    def __init__(
        self,
        inputs,
        outputs,
        ...
        super(Orchids52Resnet50STN, self).__init__(
    ...
    def config_checkpoint(self, checkpoint_dir):
        super(Orchids52Resnet50STN, self).config_checkpoint(checkpoint_dir)

    def load_from_v1(self, latest_checkpoint, target_model="resnetv2", model_name="resnet_v2_50_stn_v15", **kwargs):
        ...
        if training_for_tf25:
            localization_params = "ResnetV2"
            features_extraction = "ResnetV2"
            features_extraction_common = "ResnetV2"
        else:
            localization_params = model_name + "/localization_params/ResnetV2"
            features_extraction = model_name + "/features-extraction/ResnetV2"
            features_extraction_common = model_name + "/features-extraction-common/ResnetV2"

        local_var_loaded = super(Orchids52Resnet50STN, self).load_from_v1(
            latest_checkpoint,
        ...
        extract_var_loaded = super(Orchids52Resnet50STN, self).load_from_v1(
            latest_checkpoint,
        ...
        extract_comm_var_loaded = super(Orchids52Resnet50STN, self).load_from_v1(
            latest_checkpoint,
        ...
        for var_name in var_maps:
            if training_for_tf25:
                _key = "ResnetV2/" + var_maps[var_name]
            else:
    ...
    def save_model_variables(self):
        super(Orchids52Resnet50STN, self).save_model_variables()
    ...
    def set_resnet_training_status(self, trainable, **kwargs):
        super(Orchids52Resnet50STN, self).set_resnet_training_status(trainable, **kwargs)
    ...
    def config_layers(self, fine_tune, **kwargs):
        ...
        if training_step == TRAIN_STEP1:
            self.set_resnet_training_status(fine_tune, **kwargs)
        elif training_step == TRAIN_STEP2:
            self.set_resnet_training_status(False, **kwargs)
            ...
            self.set_resnet_training_status(True, **kwargs)
    ...
    def load_model_step1(self, **kwargs):
        ...
        if self.step > 1:
            for idx, predict_layer in enumerate(self.predict_layers):
                from_name = (
                    "branch_block_resnet50/prediction_layer/dense"
                    if idx == 0
                    else "branch_block_resnet50/prediction_layer_{}/dense_{}".format(idx, idx)
                )
    ...
    def load_model_step2(self, **kwargs):
        ...
        load_model_from_hdf5(
            ...
            optimizer=self.optimizer,
            loaded_vars=self.loaded_vars,
            to_name="resnetv2_stn_base_50_224_",
            from_name="resnetv2_global_branch_50_224_",
        )
        load_model_from_hdf5(
            filepath=latest_checkpoint,
            ...
            loaded_vars=self.loaded_vars,
            to_name="resnetv2_stn_base_50_224_",
            from_name="resnetv2_shared_branch_50_224_",
        )
    ...
    def restore_model_from_latest_checkpoint_if_exist(self, **kwargs):
        ...
        if training_step == TRAIN_STEP1 and load_from_old_format:
            ...
            if latest_checkpoint:
                ...
                if self.training:
                    try:
                        var_loaded = Orchids52Resnet50.load_from_v1(
                            self,
                            latest_checkpoint=latest_checkpoint,
                            target_model="resnetv2_stn_base_1.40_224_",
                            model_name="ResnetV2",
                            include_prediction_layer=False,
                            **kwargs,
                        )
...
def create_orchid_resnet_v2_50_15(num_classes, optimizer=None, loss_fn=None, **kwargs):
    stn_denses = None
    boundary_loss = None
    branches_block = None
    estimate_block = None
    ...
    if train_step != TRAIN_STEP1:
        ...
        branches_block = BranchBlockResnet50(num_classes=num_classes, dropout=dropout, batch_size=batch_size)
        ...
        resnet_logits = stn_base_model(processed_inputs)
        outputs = prediction_layer(resnet_logits)

    model = Orchids52Resnet50STN(
        inputs,
        outputs,
        optimizer=optimizer,
        loss_fn=loss_fn,
